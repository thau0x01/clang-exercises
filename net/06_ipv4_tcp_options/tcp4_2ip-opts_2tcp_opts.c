...
int main(int argc, char **argv)
{
    ...
    ip_opt_len[0] = 0;
    ip_options[0][0] = 68u;
    ip_opt_len[0]++; // Timestamp option
    ip_options[0][1] = 8u;
    ip_opt_len[0]++; // Option length (bytes) - 40 is maximum
    ip_options[0][2] = 9u;
    ip_opt_len[0]++; // Pointer
    ip_options[0][3] = 0u;
    ip_opt_len[0]++; // Overflow and Flag
    ip_options[0][4] = 0u;
    ip_opt_len[0]++; // Time in milliseconds (right-justified) - random here
    ip_options[0][5] = 0u;
    ip_opt_len[0]++; // Time cont'd
    ip_options[0][6] = 67u;
    ip_opt_len[0]++; // Time cont'd
    ip_options[0][7] = 78u;
    ip_opt_len[0]++; // Time cont'd
    ...
    ip_opt_len[1] = 0;
    ip_options[1][0] = (1 << 7) | 2u;
    ip_opt_len[1]++; // Option data is military security classification of datagrams
    ip_options[1][1] = 11u;
    ip_opt_len[1]++; // Option length (bytes)
    ip_options[1][2] = 107u;
    ip_opt_len[1]++; // Top Secret
    ip_options[1][3] = 197u;
    ip_opt_len[1]++; // Top Secret
    ip_options[1][4] = 0u;
    ip_opt_len[1]++; // Compartments: Not compartmented
    ip_options[1][5] = 0u;
    ip_opt_len[1]++; // Compartments: Not compartmented
    ip_options[1][6] = 67u;
    ip_opt_len[1]++; // Handling Restrictions: Critical Nuclear Weapon Design Information (CN)
    ip_options[1][7] = 78u;
    ip_opt_len[1]++; // Handling Restrictions: Critical Nuclear Weapon Design Information (CN)
    ip_options[1][8] = 65;
    ip_opt_len[1]++;
    ...
    tcp_opt_len[0] = 0;
    tcp_options[0][0] = 2u;
    tcp_opt_len[0]++; // Option kind 2 = maximum segment size
    tcp_options[0][1] = 4u;
    tcp_opt_len[0]++; // This option kind is 4 bytes long
    tcp_options[0][2] = 0x1u;
    tcp_opt_len[0]++; // Set maximum segment size to 0x100 = 256
    tcp_options[0][3] = 0x0u;
    tcp_opt_len[0]++;

    // Second TCP option - Timestamp option
    tcp_opt_len[1] = 0;
    tcp_options[1][0] = 8u;
    tcp_opt_len[1]++; // Option kind 8 = Timestamp option (TSOPT)
    tcp_options[1][1] = 10u;
    tcp_opt_len[1]++; // This option is 10 bytes long
    tcp_options[1][2] = 0x2u;
    tcp_opt_len[1]++; // Set the sender's timestamp (TSval) (4 bytes) (need SYN set to be valid)
    tcp_options[1][3] = 0x3u;
    tcp_opt_len[1]++;
    tcp_options[1][4] = 0x4u;
    tcp_opt_len[1]++;
    tcp_options[1][5] = 0x5u;
    tcp_opt_len[1]++;
    tcp_options[1][6] = 0x6u;
    tcp_opt_len[1]++; // Set the echo timestamp (TSecr) (4 bytes) (need ACK set to be valid)
    tcp_options[1][7] = 0x7u;
    tcp_opt_len[1]++;
    tcp_options[1][8] = 0x8u;
    tcp_opt_len[1]++;
    tcp_options[1][9] = 0x9u;
    tcp_opt_len[1]++;
