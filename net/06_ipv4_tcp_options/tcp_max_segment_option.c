...
int main(int argc, char **argv)
{
    ...
    int i, status, frame_length, sd, bytes, *ip_flags, *tcp_flags, opt_len;
    ...
    iphdr.ip_len = htons(IP4_HDRLEN + TCP_HDRLEN + opt_len);
    ...
    // TCP Options
    memcpy(ether_frame + ETH_HDRLEN + IP4_HDRLEN + TCP_HDRLEN, options, opt_len * sizeof(uint8_t));
