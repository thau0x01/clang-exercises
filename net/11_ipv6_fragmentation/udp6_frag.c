...
#include <netinet/in.h>      // IPPROTO_UDP, IPPROTO_FRAGMENT
...
int main(int argc, char **argv)
{
  ...
  fragbufferlen = UDP_HDRLEN + payloadlen;
  ...
  // We'll change this to 44 only in ether_frame because otherwise UDP checksum will be wrong.
  ...
  // UDP header
  memcpy(fragbuffer, &udphdr, UDP_HDRLEN * sizeof(uint8_t));
  // UDP data
  memcpy(fragbuffer + UDP_HDRLEN, payload, payloadlen * sizeof(uint8_t));

  // IPv6 next header (8 bits)
  if (nframes == 1)
  {
    iphdr.ip6_nxt = IPPROTO_UDP; // 17 for UDP
  }
  else
  {
