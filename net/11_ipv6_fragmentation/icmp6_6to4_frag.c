...
// Send an IPv6 ICMP packet through an IPv4 tunnel (6to4) via raw socket
...
#include <netinet/in.h>      // IPPROTO_IPV6, IPPROTO_ICMPV6, IPPROTO_FRAGMENT, INET_ADDRSTRLEN, INET6_ADDRSTRLEN
...
int main(int argc, char **argv)
{
  ...
  // Payload length (16 bits): fragmentable portion of packet. i.e., ICMP header + ICMP payload data
  ip6hdr.ip6_plen = htons(ICMP_HDRLEN + payloadlen);
  ...
  icmphdr.icmp6_cksum = icmp6_checksum(ip6hdr, icmphdr, payload, payloadlen);
  ...
  if (nframes == 1)
  {
    ip6hdr.ip6_nxt = IPPROTO_ICMPV6; // 58 for ICMP
  }
  else
  {
