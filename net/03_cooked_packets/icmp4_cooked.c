...
#include <netinet/in.h>      // IPPROTO_RAW, IPPROTO_ICMP, INET_ADDRSTRLEN
...
int main(int argc, char **argv)
{
    ...
    frame_length = IP4_HDRLEN + ICMP_HDRLEN + datalen;

    // IPv4 header
    memcpy(ether_frame, &iphdr, IP4_HDRLEN);

    // ICMP header
    memcpy(ether_frame + IP4_HDRLEN, &icmphdr, ICMP_HDRLEN);

    // ICMP data
    memcpy(ether_frame + IP4_HDRLEN + ICMP_HDRLEN, data, datalen);

    // Calculate ICMP checksum
    icmphdr.icmp_cksum = checksum((uint16_t *)(ether_frame + IP4_HDRLEN), ICMP_HDRLEN + datalen);
    memcpy(ether_frame + IP4_HDRLEN, &icmphdr, ICMP_HDRLEN);
