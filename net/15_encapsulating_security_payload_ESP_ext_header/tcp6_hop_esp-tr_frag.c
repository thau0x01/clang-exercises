...
main (int argc, char **argv) {
  ...
  esp_paylen = TCP_HDRLEN + payloadlen;
  esp_padlen = 0;
  ...
  // Build ESP payload (TCP header, TCP payload data, padding).
  ...
  // ESP trailer.
  esptail.pad_len = esp_paylen - TCP_HDRLEN - payloadlen;  // Amount of padding for ESP payload
  esptail.nxt_hdr = IPPROTO_TCP;  // Next header is TCP header.
  ...
  if (hbh_nopt > 0) {
    ...
    if (nframes == 1)  {
      hophdr.nxt_hdr = IPPROTO_ESP;  // 50 for encapsulating security payload (ESP) extension header
    } else {
    ...
    if (nframes == 1)  {
      iphdr.ip6_nxt = IPPROTO_ESP;  // 50 for encapsulating security payload (ESP) extension header
    } else {
