...
// The authentication header is used here in transport mode.
...
#include <netinet/in.h>       // IPPROTO_HOPOPTS, IPPROTO_AH, IPPROTO_TCP, IPPROTO_FRAGMENT, INET6_ADDRSTRLEN
...
main (int argc, char **argv) {
  ...
  fragbufferlen = ATH_HDRLEN + auth_len + TCP_HDRLEN + payloadlen;
  ...
  // TCP data
  memcpy (fragbuffer + ATH_HDRLEN + auth_len + TCP_HDRLEN, payload, payloadlen * sizeof (uint8_t));

  // IPv6 next header (8 bits)
  if (hbh_nopt > 0) {
    iphdr.ip6_nxt = IPPROTO_HOPOPTS;
    if (nframes == 1)  {
      hophdr.nxt_hdr = IPPROTO_AH;  // 51 for authentication extension header
    } else {
      hophdr.nxt_hdr = IPPROTO_FRAGMENT;  // 44 for Fragmentation extension header
    }

  } else {
    if (nframes == 1)  {
      iphdr.ip6_nxt = IPPROTO_AH;  // 51 for authentication extension header
    } else {
